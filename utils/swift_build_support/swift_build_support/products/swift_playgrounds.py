...
class SwiftPlaygrounds(product.Product):
    ...
    def product_source_name(cls):
        return "swift-play-experimental"
    ...
    def should_clean(self, host_target):
        # Workaround for 'swift-play' not detecting compiler/stdlib changes.
        return True
...
class SwiftPlaygroundsCMakeShim(cmake_product.CMakeProduct):
    ...
    def build(self, host_target):
        ...
        self.cmake_options.define('SwiftPlaygrounds_MACRO', 'NO')

        # Specify path to swift-testing build dir for libTestDiscovery module.
        build_root = os.path.dirname(self.build_dir)
        swifttesting_build_dir = os.path.join(
            '..', build_root, '%s-%s' % ('swifttesting', host_target))
        self.cmake_options.define('SWIFT_TESTING_BUILD_DIR', swifttesting_build_dir)
